...
pub fn to_source(block: &decompile::ast::Block) -> Result<String, LuaError> {
    ...
    assert_parse("raw full_moon display", &raw)?;
...
fn format_source(raw: String) -> Result<String, LuaError> {
    run_with_large_stack("stylua format", move || {
        format_code(&raw, Config::default(), None, OutputVerification::Full)
...
fn assert_parse(stage: &'static str, source: &str) -> Result<(), LuaError> {
    ...
    let stage_label = stage.to_owned();
    run_with_large_stack(stage, move || {
        full_moon::parse(&source)
            .map(|_| ())
            .map_err(|errors| format!("{stage_label} did not parse: {errors:#?}"))
    })
}
