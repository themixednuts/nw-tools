...
use nw_serialize_codegen::{
    ...
    NetworkTypeCapability, NetworkWireShape, ReflectedTypeCatalogSummary, RustCodegenPlanner,
    ...
    SerializeCodegenSelectionManifest, SerializeCodegenUnit, SerializeContextCompileInputs,
...
struct NetworkRustArgs {
    ...
    kind: NetworkRustKind,
    /// Replicated-state type index to emit. Repeat as needed; omitted means every proven state.
...
fn network_schema(args: &NetworkSchemaArgs, status: CodegenStatus) -> Result<()> {
    ...
    for overlay_path in args.ghidra_reports.iter().skip(1) {
        ...
        println!(
            ...
            merge.replaced_field_registration_function_count,
            merge.added_field_registration_function_count,
        );
        progress.advance_with_message(
            1,
            Some(format!("merged Ghidra overlay {}", overlay_path.display())),
        );
    }
    ...
    reject_compile_errors(&compile_unit)?;
    let catalog_merge = schema.merge_serialize_type_catalog(&compile_unit.catalog);
    ...
    println!(
        "serialize: {} source type(s), {} matched ({} by type id, {} by name, {} ambiguous name), {} filled name(s), {} required generic type(s) resolved",
        ...
        catalog_merge.matched_generic_type_count,
    );
...
fn network_rust(args: &NetworkRustArgs, status: CodegenStatus) -> Result<()> {
    ...
    {
        bail!("Rust source audit is only available for descriptor emission");
    }
    ...
    let output = match args.kind {
        NetworkRustKind::Descriptors => {
            NetworkRustEmitter::emit_descriptors_with_context(&schema, &context)
                .context("emit network Rust descriptor source")?
        }
        NetworkRustKind::States => {
            let type_indices = if args.type_indices.is_empty() {
                schema
                    .types
                    .iter()
                    .filter(|network_type| {
                ...
                args.type_indices.clone()
            };
            NetworkRustEmitter::emit_replicated_states_with_options_and_context(
                &schema,
                type_indices,
                NetworkReplicatedStateEmitOptions::default(),
                &context,
            )
            .context("emit replicated-state Rust source")?
        }
        NetworkRustKind::Messages => {
            NetworkRustEmitter::emit_messages_with_context(&schema, &context)
                .context("emit message Rust source")?
        }
    };
    ...
    println!(
        "network rust: kind={:?}, {} descriptor(s), {} state(s), {} message(s), {} support type(s)",
        args.kind,
        output.report.descriptor_count,
        output.report.replicated_state_count,
        output.report.message_count,
        output.report.support_type_count,
    );
