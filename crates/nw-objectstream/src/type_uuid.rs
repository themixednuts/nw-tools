//! ObjectStream type-id folding.
//!
//! ObjectStream type IDs use the native `AZ::Uuid::CreateName` /
//! `CreateData` behavior — the primitive itself (SHA-1 the input bytes
//! directly, take the first 16 digest bytes, apply the version/variant
//! masks) lives in [`nw_asset::uuid`], since `nw-objectstream` already
//! depends on `nw-asset`. This module only folds template type IDs over
//! that primitive: `lhs || rhs` via `combine` (`AZ::Uuid::operator+`).
//!
//! [`nw_asset::uuid::AzUuidExt`] is re-exported here for consumers that want
...
pub use nw_asset::uuid::AzUuidExt;

use nw_asset::uuid::const_impl::{combine, sha1_padded_len, uuid_from_bytes, write_be_u32};
